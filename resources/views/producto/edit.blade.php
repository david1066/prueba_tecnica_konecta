...
<form  method="post" action="{{ url('/producto/update/'.$producto->id) }}">
  ...
  Ir al listado de productos</a>
    ...
    <div class="form-group">
        <label>Precio</label>
        <input type="number" min="0"  step="1"  class="form-control" name="precio" id="precio" value="{{$producto->precio}}" required>
    </div>


   <div class="form-group">
        <label>Peso</label>
        <input type="number" min="1"  step="1" class="form-control" name="peso" id="peso" value="{{$producto->peso}}" required>
    </div>
    <div class="form-group">
       ...
       <select name="categoria_id" class="form-control" required id="categoria_id">
           ...
           @foreach($categoria as $value)
            ...
            @if($value->id==$producto->categoria_id)
    ...
    <div class="form-group">
        <label>Stock</label>
        <input type="number" min="1"  step="1" class="form-control" name="stock" value="{{$producto->stock}}" id="stock" required>
    </div>
